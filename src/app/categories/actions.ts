...
export async function createCategory(_prev: string | undefined, formData: FormData) {
  ...
  const { error } = await supabaseAdmin().from("categories").insert({ name, type, color, icon });
...
export async function renameCategory(id: string, name: string, color: string, icon: string) {
  if (!name.trim()) throw new Error("Nama wajib diisi.");
  const { error } = await supabaseAdmin()
    .from("categories")
    .update({ name: name.trim(), color, icon: icon.trim() || "circle" })
    .eq("id", id);
